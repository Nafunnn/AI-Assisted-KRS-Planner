...
class AiProviderManager
{
    public function driver(?AiProviderConfig $config = null): AiProviderDriver
    {
        ...
        return match ($config->provider) {
            AiProvider::OpenAi => new OpenAiCompatibleDriver($this->client($config), $config->default_model ?? 'gpt-4o-mini'),
            ...
            AiProvider::OpenRouter => new OpenAiCompatibleDriver($this->client($config), $config->default_model ?? 'openai/gpt-4o-mini'),
